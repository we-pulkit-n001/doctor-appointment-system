...
let model_namespace = 'VaahCms\\Modules\\Assignment\\Models\\Doctor';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/assignment/doctors";
...
export const useDoctorStore = defineStore({
    ...
    actions: {
        ...
        async exportDoctorData(){
            let file_data = null;
            try {
                await vaah().ajax(
                    this.ajax_url.concat('/doctors/export'),
                    (data, res) => {
                        file_data = res.data;
                    }
                );
        ...
        async getUniqueSpecializations(){
            await vaah().ajax(
                ...
                (data,res) => {
                    ...
                    this.max_price_display = parseInt(res.data.max_price, 10) || 1000;
                    this.query.filter.price_range = [10, this.max_price_display];
                }
            );
        }
        //---------------------------------------------------------------------
