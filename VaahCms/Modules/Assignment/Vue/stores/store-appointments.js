...
let model_namespace = 'VaahCms\\Modules\\Assignment\\Models\\Appointment';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/assignment/appointments";
...
export const useAppointmentStore = defineStore({
    ...
    actions: {
        ...
        async exportAppointmentsData(){
                let file_data = null;
                try {
                    await vaah().ajax(
                        this.ajax_url.concat('/appointments/export'),
                        (data, res) => {
                            file_data = res.data;
                        }
                    );
